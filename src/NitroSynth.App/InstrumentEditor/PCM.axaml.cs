...
{
    public partial class PCM : Window
    {
        public PCM()
        {
            InitializeComponent();
        ...
        public PCM(byte[] articulationBytes, string title = "PCM Instrument Editor")
            : this() 
        {
        ...
        {
            public string Title { get; set; } = "PCM Instrument Editor";
            ushort _swavId; public ushort SwavId { get => _swavId; set { _swavId = value; OnPropertyChanged(); } }
            ushort _swarId; public ushort SwarId { get => _swarId; set { _swarId = value; OnPropertyChanged(); } }
            ...
            {
                if (bytes == null || bytes.Length < 0x0A) throw new ArgumentException("Articulation must be 10 bytes.");
                SwavId = BitConverter.ToUInt16(bytes, 0x00);
                SwarId = BitConverter.ToUInt16(bytes, 0x02);
                BaseKey = bytes[0x04];
            ...
            public byte[] ToBytes()
            {
                ...
                BitConverter.TryWriteBytes(buf.AsSpan(0x00, 2), SwavId);
                BitConverter.TryWriteBytes(buf.AsSpan(0x02, 2), SwarId);
                buf[0x04] = BaseKey;
                buf[0x05] = AttackRate;
                buf[0x06] = DecayRate;
